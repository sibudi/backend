...
//        log.info("-------------------- read02 DataSourceTwo init ---------------------");
    ...
    @Bean(name = "riskBackupDatasource")
    @ConfigurationProperties(prefix = "mysql.datasource.riskBackup")
    public DataSource riskBackUpDB() {
//        log.info("-------------------- riskBackup DataSource init ---------------------");
        return DataSourceBuilder.create().type(dataSourceType).build();
    }
